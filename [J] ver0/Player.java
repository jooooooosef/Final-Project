...
public class Player implements DrawingObject{
    ...
    public boolean isCollidingWithLeftOfFrame(GameCanvas gv){
        return (this.xCoordinate <= 0);
    }
    public boolean isCollidingWithRightOfFrame(GameCanvas gv){
        return (this.xCoordinate + (sprite.getWidth()*scale) >= gv.getWidth());
    }

    public boolean isCollidingWithTopOfFrame(GameCanvas gv){
        return (this.yCoordinate <= 0);
    }

    public boolean isCollidingWithBottomOfFrame(GameCanvas gv){
        return (this.yCoordinate + (sprite.getHeight()*scale) >= gv.getHeight());
    }
